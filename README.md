...
When user types criteria in multi-filter and then press enter then table displays only rows that matches the user input. From the picture below we can see only data that has been filtered based on the user input – that is -- *State: ca, Shape: triangle and Date: 1/1/2010*.
...
<p align="center">
<i>New table, displays only the results that matches the users input.</i>
</p>
...
<p align="center">
<i>Reset filter and refresh page button at the top left corner of the web-page.</i>
</p>
...
- Insted of "UFO Sightings" reset link, we can add some functioss which will reset page automatically. Also provide some "TOP" and "BOTTOM" functions to scroll page up and down to make it user friendly.
...
- **Adding drop down-menu from the filter.** Also Sometimes users don’t know what exactly they are looking for in just exploring the web-page and its data. So We can increase the positive apporach of user by adding drop-down menu,for the ease insted of guessing the options.
